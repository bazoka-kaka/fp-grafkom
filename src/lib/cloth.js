...
import {arrayToMatrix, computeFaceNormal} from "./index";
...
class Cloth {
  ...
  updateWindForce(delta) {
    const time = Date.now();
    this.windStrength = Math.cos(time / 700000000) * 500 + 1600;
    this.windForce.set(
      Math.sin(time / 2000),
      Math.cos(time / 3000),
      Math.sin(time / 1000)
    ...
    if (this.windEnabled) {
      ...
      faces.forEach((face) => {
        ...
        this.tmpForce
          .copy(faceNormal)
          .normalize()
          .multiplyScalar(faceNormal.dot(this.windForce));

        face.map(i => {
          this.particles[i].addForce(this.tmpForce);
        })
      });
  ...
  getParticles() {
    return this.particles.map(val => [val.position.x, val.position.y, val.position.z]).flat()
  }
}
